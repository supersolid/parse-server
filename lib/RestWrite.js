...
RestWrite.prototype.validateClientClassCreation = function () {
  ...
  var sysClass = Schema.systemClasses;
...
RestWrite.prototype.transformUser = function () {
  ...
  if (!this.query) {
    ...
    promise = promise.then(function () {
      var expiresAt = _this7.config.generateSessionExpiresAt();
      var sessionData = {
        sessionToken: token,
        user: {
          __type: 'Pointer',
        ...
        restricted: false,
        installationId: _this7.auth.installationId,
  ...
  if (this.query && this.auth.user && this.auth.user.getSessionToken()) {
    _cache2.default.users.remove(this.auth.user.getSessionToken());
  }
  ...
  if (this.query) {
    ...
    return this.config.database.update(this.className, this.query, this.data, this.runOptions).then(function (resp) {
      ...
      if (_this10.storage['changedByTrigger']) {
        resp = Object.keys(_this10.data).reduce(function (memo, key) {
          memo[key] = resp[key] || _this10.data[key];
          return memo;
        }, resp);
      }
      _this10.response = {
    ...
    // Run a create
    return this.config.database.create(this.className, this.data, this.runOptions).then(function (resp) {
      Object.assign(resp, {
