...
function injectDefaultSchema(schema) {
  var defaultSchema = Schema.defaultColumns[schema.className];
  if (defaultSchema) {
    Object.keys(defaultSchema).forEach(function (key) {
...
function getAllSchemas(req) {
  return req.config.database.schemaCollection().then(function (collection) {
    ...
    return schemas.map(injectDefaultSchema);
  }).then(function (schemas) {
...
function getOneSchema(req) {
  ...
  }).then(injectDefaultSchema).then(function (schema) {
