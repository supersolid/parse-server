      ...
      if (body.data && body.data.badge) {
        (function () {
          ...
          badgeUpdate = function badgeUpdate() {
            var badgeQuery = new _RestQuery2.default(config, auth, '_Installation', updateWhere);
            return badgeQuery.buildRestWhere().then(function () {
              var restWhere = (0, _deepcopy2.default)(badgeQuery.restWhere);
              ...
              return config.database.adaptiveCollection("_Installation").then(function (coll) {
                return coll.updateMany(restWhere, op);
              });
            });
          };
        })();
    ...
    value: function sendToAdapter(body, installations, pushStatus, config) {
      ...
      if (body.data && body.data.badge && body.data.badge == "Increment") {
        var _ret2 = function () {
          ...
          return {
            v: Promise.all(promises).then(function (results) {
              if (Array.isArray(results)) {
                return Promise.resolve(results.reduce(function (memo, result) {
