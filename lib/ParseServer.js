...
var ParseServer = function () {
  function ParseServer(_ref) {
    ...
    var usernameUniqueness = databaseController.adapter.ensureUniqueness('_User', ['username'], requiredUserFields).catch(function (error) {
      _logger.logger.warn('Unable to ensure uniqueness for usernames: ', error);
      return Promise.reject();
    });

    var emailUniqueness = databaseController.adapter.ensureUniqueness('_User', ['email'], requiredUserFields).catch(function (error) {
