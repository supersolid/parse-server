  ...
  _createClass(PostgresStorageAdapter, [{
    ...
    value: function createClass(className, schema) {
      ...
      return this._ensureSchemaCollectionExists().then(function () {
        return _this._client.none('CREATE TABLE $1:name (' + patternsArray.join(',') + ')', [className].concat(valuesArray));
        ...
        return _this._client.none('INSERT INTO "_SCHEMA" ("className", "schema", "isParseClass") VALUES ($<className>, $<schema>, true)', { className: className, schema: schema });
      }).then(function () {
        return schema;
      });
    }
    ...
    value: function deleteAllClasses() {
      ...
      return this._client.any('SELECT "className" FROM "_SCHEMA"').then(function (results) {
    ...
    value: function deleteObjectsByQuery(className, schema, query) {
      return this._client.one('WITH deleted AS (DELETE FROM $<className:name> RETURNING *) SELECT count(*) FROM deleted', { className: className }, function (a) {
        return +a.count;
      }).then(function (count) {
        if (count === 0) {
    ...
    value: function find(className, schema, query, _ref3) {
      ...
      return this._client.any(qs, values).then(function (results) {
