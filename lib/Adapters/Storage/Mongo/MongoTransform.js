...
function transformWhere(schema, className, restWhere) {
  ...
  for (var restKey in restWhere) {
    ...
    if (restKey === 'user' && out.key !== '_p_user') {
      _logger2.default.info('MongoTransform.transformWhere', 'className', className, 'restKey', restKey, 'out', out, 'restWhere', restWhere, 'schema.data', schema.data);
    }
  }
  return mongoWhere;
}
          ...
          default:
            ...
            if (key.indexOf('_p_') == 0) {
              ...
              if (!expected) {
                ...
                _logger2.default.info('transform.js', 'typeof schema', typeof schema === 'undefined' ? 'undefined' : _typeof(schema));
                if (schema) {
                  _logger2.default.info('transform.js', 'typeof schema.getExpectedType', _typeof(schema.getExpectedType));
                  _logger2.default.info('transform.js', 'schema.data', schema.data);
                }
                break;
              }
