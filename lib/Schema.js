...
exports.defaultColumns = exports.systemClasses = exports.buildMergedSchemaObject = exports.schemaAPITypeToMongoFieldType = exports.invalidClassNameMessage = exports.classNameIsValid = exports.load = undefined;
...
var Schema = function () {
  ...
  _createClass(Schema, [{
    ...
    value: function reloadData() {
      ...
      this.data = {};
      this.perms = {};
      return this._collection.getAllSchemas().then(function (allSchemas) {
        allSchemas.forEach(function (schema) {
          var parseFormatSchema = _extends({}, defaultColumns._Default, defaultColumns[schema.className] || {}, schema.fields);
          ...
          _this.data[schema.className] = _lodash2.default.mapValues(parseFormatSchema, function (parseField) {
    ...
    value: function addClassIfNotExists(className, fields, classLevelPermissions) {
      ...
      return this._collection.addSchema(className, mongoObject.result).catch(function (error) {
        if (error === undefined) {
          throw new Parse.Error(Parse.Error.INVALID_CLASS_NAME, 'Class ' + className + ' already exists.');
        } else {
    ...
    value: function updateClass(className, submittedFields, classLevelPermissions, database) {
      ...
      .then(function () {
        return _MongoSchemaCollection2.default._TESTmongoSchemaToParseSchema(mongoObject.result);
      });
    }
    ...
    value: function validateField(className, fieldName, type, freeze) {
      ...
      var expected = this.data[className][fieldName];
      if (expected) {
        ...
        var _ret = function () {
          ...
          return {
            v: Object.keys(classData).filter(function (field) {
              ...
              var className = type.slice('relation<'.length, type.length - 1);
              memo[field] = {
                __type: 'Relation',
                className: className
              };
...
function buildMergedSchemaObject(mongoObject, putRequest) {
  ...
  for (var oldField in mongoObject) {
    if (oldField !== '_id' && oldField !== 'ACL' && oldField !== 'updatedAt' && oldField !== 'createdAt' && oldField !== 'objectId') {
      ...
      if (!fieldIsDeleted) {
        newSchema[oldField] = _MongoSchemaCollection2.default._DONOTUSEmongoFieldToParseSchemaField(mongoObject[oldField]);
      }
    }
  }
